...
<%
    ...
    if(rs.next()){
        String q1="UPDATE complaints SET case_reg='Filed as FIR' WHERE sus_name='" + name + "' && date='" + date + "' && category='" + category + "'&& incident='" + location + "'";
        int rowCount=adminDao.UpdateComplaint(q1);
        if(rowCount>0){
            request.getSession();
            session.setAttribute("success", "Filed as FIR Successfully!");
            response.sendRedirect("viewComplaints.jsp");
        }else{
            request.getSession();
            session.setAttribute("failed", "Failed to File as FIR !");
            response.sendRedirect("viewComplaints.jsp");
        }
    }else{
        request.getSession();
